...
config = {
    "input_size": len(discretizer_header),
    "hidden_size": 64,
    "num_layers": 2,
...
train_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'train'), listfile=os.path.join(args.data, 'train_listfile.csv'), period_length=48.0)
val_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'train'), listfile=os.path.join(args.data, 'val_listfile.csv'), period_length=48.0)
test_reader = InHospitalMortalityReader(dataset_dir=os.path.join(args.data, 'test'), listfile=os.path.join(args.data, 'test_listfile.csv'), period_length=48.0)
...
model.final_name = args.model_name
...
if args.mode == 'train':
    for epoch in range(n_trained_chunks + config["num_epochs"]):
        model.train()
        train_loss = 0
    ...
    utils.save_results(test_names, test_preds, test_labels, os.path.join(args.output_dir, "test_predictions", os.path.basename(args.load_state)) + ".csv")
